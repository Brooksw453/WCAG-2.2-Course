...
interface ChatMessage {
  role: 'user' | 'assistant';
  content: string;
  isDraft?: boolean; // marks messages that contain generated draft content
}
...
export default function DraftChat({
  ...
  }, [isOpen, messages.length, sectionTitle, currentDraft]);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/assignment/draft-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assignmentId,
      ...
      if (!res.ok) {
        ...
        const isDraft = data.hasDraft === true;
        setMessages(prev => [
          ...prev,
          { role: 'assistant', content: data.message, isDraft },
        ]);
        // If the AI generated a draft, also store it for the insert button
        if (isDraft && data.draftContent) {
          setMessages(prev => {
            const last = prev[prev.length - 1];
            return [...prev.slice(0, -1), { ...last, content: data.message, isDraft: true }];
          });
        }
      }
    } catch {
  ...
  const extractDraftContent = (message: string): string => {
    // Extract content between --- DRAFT --- markers, or the whole message if no markers
    const draftMatch = message.match(/---\s*DRAFT\s*---\n([\s\S]*?)\n---\s*END DRAFT\s*---/);
    if (draftMatch) return draftMatch[1].trim();
    // Fallback: try to find content after "Here's a draft:" or similar
  ...
  return (
    <>
      {/* Backdrop — click-to-close; keyboard users have Escape + close button */}
      {isOpen && (
        <div
          aria-hidden="true"
      ...
      >
        ...
        <div className="bg-gradient-to-r from-purple-600 to-indigo-600 px-4 py-3 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-purple-200 truncate max-w-[200px] sm:max-w-[280px]">{sectionTitle}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="bg-purple-50 dark:bg-purple-900/30 border-b border-purple-100 dark:border-purple-800 px-4 py-2 flex-shrink-0">
          <p className="text-xs text-purple-700 dark:text-purple-300">
            <span className="font-semibold">How it works:</span> Answer my questions, and I&apos;ll draft your response. Click &quot;Use This Draft&quot; to insert it into your writing area, then refine and submit for AI Coaching.
          </p>
        </div>
              ...
              <div className={`max-w-[90%] ${msg.role === 'user' ? '' : ''}`}>
                ...
                {msg.isDraft && msg.role === 'assistant' && (
                  <div className="mt-2 flex gap-2">
                    ...
                    <span className="text-xs text-gray-400 self-center">Inserts into your response box</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {draftInserted && (
            <div className="flex justify-center">
              <div className="bg-green-100 text-green-700 text-xs font-medium px-3 py-1.5 rounded-full flex items-center gap-1">
                ...
                Draft inserted into your response!
              </div>
            </div>
          )}
